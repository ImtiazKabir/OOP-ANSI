...
export DEBUG_OPTIMIZATION    :=-g3 -fno-inline \
                               -fno-zero-initialized-in-bss 
ifeq ($(CC),gcc)
	DEBUG_OPTIMIZATION+=-fno-guess-branch-probability -fno-function-cse -fno-defer-pop 
endif
...
checkDebug checkRelease:
	cppcheck --enable=all --addon=misra.json --inconclusive -v --quiet -isrc/webcam $(SRCDIR) $(INCDIR)
...
.PHONY:diffcheck
diffcheck:
	valgrind Debug/bin/main sample\ without\ error\ and\ warning/input.txt
	diff -w token.txt sample\ without\ error\ and\ warning/token.txt
	diff -w log.txt sample\ without\ error\ and\ warning/log.txt
	valgrind Debug/bin/main sample\ with\ error/input.txt
	diff -w token.txt sample\ with\ error/token.txt
	diff -w log.txt sample\ with\ error/log.txt
	valgrind Debug/bin/main sample\ with\ warning/input.txt
	diff -w token.txt sample\ with\ warning/token.txt
	diff -w log.txt sample\ with\ warning/log.txt
	valgrind Debug/bin/main anik/input.txt
	diff -w token.txt anik/token.txt
	diff -w log.txt anik/log.txt
